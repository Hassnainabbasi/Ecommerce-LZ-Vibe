...
// 🔴 Alag se Image Slider Component (Taake har card ka slide independently kaam kare)
function ProductImageSlider({ imageSource, altText }) {
  const [currentIndex, setCurrentIndex] = useState(0);

  // Agar image array hai to use lein, warna single string ko array bana dein
  const images = Array.isArray(imageSource)
    ? imageSource
    : [imageSource].filter(Boolean);
  ...
  const safeSrc =
    typeof currentImg === "string"
      ? getImageUrl(currentImg)
      : "https://placehold.co/600x400?text=No+Image";

  return (
    <div className="relative group bg-slate-100 h-44 w-full flex items-center justify-center overflow-hidden">
      {/* Main Feature / Current Image */}
      <img
        src={safeSrc}
      ...
      {/* Navigation Buttons (Sirf tabhi dikhen ge jab 1 se zyada images hon) */}
      {images.length > 1 && (
        <>
          <button
            onClick={handlePrev}
          ...
          {/* Dots Indicator */}
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 bg-black/30 px-2 py-0.5 rounded-full">
            {images.map((_, index) => (
              <span
                key={index}
...
export default function AllProducts() {
  ...
  const openEditModal = (product) => {
    setCurrentProduct({
      ...
      price: product.price || "",
      category: product.category || "",
      flavor: Array.isArray(product.flavor)
        ? product.flavor.join(", ")
        : product.flavor || "",
      weight: product.weight || "",
      compareAtPrice: product.compareAtPrice || "",
      ...
      offerEndsAt: product.offerEndsAt
        ? String(product.offerEndsAt).slice(0, 10)
        : "",
      isFeatured: Boolean(product.isFeatured),
      image: product.image || "",
      imageFile: null,
      ...
      <div className="mb-6 rounded-[2rem] bg-gradient-to-r from-slate-950 via-red-900 to-red-600 p-6 text-white shadow-2xl">
        ...
        <div className="mt-2 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/70">
              Edit prices, real discount badges, weekly offers, featured status,
              and catalogue data.
            </p>
          </div>
          <div className="bg-white/95 border border-white/20 rounded-full px-10 relative w-full sm:w-72">
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => {
            ...
            return (
              <div
                key={product._id}
                className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-gray-800 shadow-sm transition hover:-translate-y-1 hover:border-red-200 hover:shadow-xl"
              >
                <div className="relative bg-slate-100">
                  {offer.hasOffer && (
                    <span className="absolute left-3 top-3 z-10 rounded-full bg-red-600 px-3 py-1 text-xs font-black uppercase tracking-wide text-white">
                ...
                <div className="p-4">
                  <h3 className="line-clamp-2 min-h-[56px] text-lg font-black text-slate-950">
                    {product.name}
                  </h3>
                  ...
                  <div className="my-3 flex flex-wrap items-baseline gap-2">
                    ...
                    {offer.hasOffer &&
                      offer.compareAtPrice > Number(product.price || 0) && (
                        <p className="text-sm font-bold text-slate-400 line-through">
                          Rs {offer.compareAtPrice.toLocaleString()}
                        </p>
                      )}
          ...
          <div className="text-gray-800 col-span-full bg-white border-2 border-slate-200 rounded-lg p-6 text-center">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div>
              ...
              {currentProduct.imageFile ? (
                ...
                /* Edit modal ke liye hum Array ki sirf pehli image bator preview dikhayenge */
                <img
                  src={
                    getImageUrl(
